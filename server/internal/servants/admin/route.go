...
import (
	...
	adminService "github.com/vicishero/NaiL/server/internal/service/admin"
...
func RegisterAdminRoutes(e *gin.Engine, basePath string) {
	...
	baseGroup := adminGroup.Group("/base")
	{
		...
		// WebSocket连接
		baseGroup.GET("/ws", s.WebSocketHandler)
	}
	// JWT黑名单接口（退出登录用，无需认证）
...
func registerGvaPermissionRoutes(g *gin.RouterGroup, s *AuthServant) {
	...
	{
		sysDictGroup.POST("/createSysDictionary", s.CreateSysDictionary)
		sysDictGroup.POST("/deleteSysDictionary", s.DeleteSysDictionary)
		sysDictGroup.POST("/updateSysDictionary", s.UpdateSysDictionary)
		sysDictGroup.GET("/getSysDictionaryList", s.GetSysDictionaryList)
		sysDictGroup.GET("/findSysDictionary", s.FindSysDictionary)
	...
	{
		sysParamsGroup.POST("/createSysParams", s.CreateSysParams)
		sysParamsGroup.POST("/deleteSysParams", s.DeleteSysParams)
		sysParamsGroup.POST("/deleteSysParamsByIds", s.DeleteSysParamsByIds)
		sysParamsGroup.POST("/updateSysParams", s.UpdateSysParams)
		sysParamsGroup.GET("/getSysParamsList", s.GetSysParamsList)
		sysParamsGroup.GET("/findSysParams", s.FindSysParams)
	...
	{
		sysApiTokenGroup.POST("/createApiToken", s.CreateApiToken)
		sysApiTokenGroup.POST("/deleteApiToken", s.DeleteApiToken)
		sysApiTokenGroup.POST("/deleteApiTokenByIds", s.DeleteApiTokenByIds)
		registerGetPost(sysApiTokenGroup, "/getApiTokenList", s.GetApiTokenList)
	}

	// ---- 登录日志 ----
	sysLoginLogGroup := g.Group("/sysLoginLog")
	{
		sysLoginLogGroup.POST("/deleteLoginLog", s.DeleteLoginLog)
		sysLoginLogGroup.POST("/deleteLoginLogByIds", s.DeleteLoginLogByIds)
		sysLoginLogGroup.GET("/getLoginLogList", s.GetLoginLogList)
		sysLoginLogGroup.GET("/findLoginLog", s.FindLoginLog)
	}
	...
	{
		sysVersionGroup.POST("/createSysVersion", s.CreateSysVersion)
		sysVersionGroup.POST("/deleteSysVersion", s.DeleteSysVersion)
		sysVersionGroup.POST("/deleteSysVersionByIds", s.DeleteSysVersionByIds)
		sysVersionGroup.POST("/updateSysVersion", s.UpdateSysVersion)
		sysVersionGroup.GET("/getSysVersionList", s.GetSysVersionList)
		sysVersionGroup.GET("/findSysVersion", s.FindSysVersion)
		sysVersionGroup.POST("/exportVersion", s.ExportVersion)
		sysVersionGroup.POST("/importVersion", s.ImportVersion)
		sysVersionGroup.GET("/downloadVersionJson", s.DownloadVersionJson)
	}
