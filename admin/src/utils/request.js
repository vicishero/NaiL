  ...
  (error) => {
    ...
    if (error.response.status === 401 || error.response.status === 403) {
      emitter.emit('show-error', {
        ...
        message: error.response.status === 401 ? '登录已过期，请重新登录' : '没有权限访问该资源',
        fn: () => {
          const userStore = useUserStore()
      ...
      // 认证错误不抛出全局异常，避免触发错误上报的无限循环
      return Promise.resolve({ code: error.response.status, msg: error.response.data.msg || '认证失败' })
    }
